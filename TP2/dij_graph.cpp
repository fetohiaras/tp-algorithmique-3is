...
void question_5_4(ifstream &fin, ofstream &fout) {
    fout << "=== Question 5.4 : Chemins les plus courts depuis une source ===\n";

    int n, m, start, dest_unused;
    fin >> n >> m >> start >> dest_unused;

    if (start < 1 || start > n) {
        fout << "Erreur : sommet de départ hors limites\n";
        return;
    }
